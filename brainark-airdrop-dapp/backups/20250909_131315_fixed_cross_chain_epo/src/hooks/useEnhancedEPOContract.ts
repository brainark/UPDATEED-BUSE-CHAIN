...
export const useEnhancedEPOContract = (contractAddress?: string): EPOContractData => {
  ...
  const fetchContractStats = useCallback(async () => {
    try {
      ...
      // Check if contract exists on production BrainArk network
      const code = await publicClient.getBytecode({ address: epoAddress })
      console.log('Contract bytecode check:', { address: epoAddress, chainId, codeExists: !!code && code !== '0x' })
      
      if (!code || code === '0x') {
        console.warn('Contract not found on current network, using API fallback')
  ...
  const purchaseBAK = useCallback(async (
    ...
    try {
      ...
      if (paymentToken === 'ETH' || paymentToken === 'BNB' || paymentToken === 'MATIC' || paymentToken === 'BAK') {
        // Native token payment
        const weiAmount = parseEther(paymentAmount)
        
        try {
          // Estimate gas for native token transaction
          const gasEstimate = await publicClient!.estimateGas({
            account: userAddress,
            to: epoAddress,
            value: weiAmount,
          }).catch(() => 300000n) // Default gas if estimation fails

          // Check balance before transaction
          const balance = await publicClient!.getBalance({ address: userAddress })
          const totalCost = weiAmount + (gasEstimate * 20000000000n) // Estimate gas cost
          ...
          console.error('Native token transaction failed:', nativeError)
          throw new Error('Native token payment failed: ' + nativeError.message)
        }
        ...
        if (allowance < tokenAmount) {
          try {
            const approveHash = await walletClient.writeContract({
              ...
              gas: 100000n, // Add gas limit for approval
            })
        ...
        try {
          ...
          const gasEstimate = await publicClient!.estimateContractGas({
            ...
            account: userAddress,
          }).catch(() => 500000n) // Default gas if estimation fails
