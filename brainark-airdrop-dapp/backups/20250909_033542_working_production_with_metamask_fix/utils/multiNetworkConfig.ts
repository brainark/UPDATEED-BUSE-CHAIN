...
export const PAYMENT_TOKENS: TokenConfig[] = [
  ...
  },
  {
    ...
    decimals: 6,
    contractAddress: process.env.USDC_ETHEREUM_CONTRACT || '0xA0b86a33E6441E02aaBBD88816953560998637e7',
...
export const getTreasuryAddress = (tokenSymbol: string, networkName: string): string | null => {
  // BrainArk network doesn't have USDT/USDC tokens, only BAK
  if (networkName === 'brainark' && (tokenSymbol === 'USDT' || tokenSymbol === 'USDC')) {
    console.warn(`Invalid treasury lookup: ${tokenSymbol} doesn't exist on BrainArk network`)
    return null
  }
