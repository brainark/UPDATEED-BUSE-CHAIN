...
const createSafeConnectors = () => {
  ...
  if (!isBuildTime() && !isServerSide()) {
    try {
      // Add WalletConnect with single initialization
      connectors.push(
        walletConnect({ 
          projectId,
          metadata: {
      ...
      // Coinbase Wallet disabled to prevent API timeout issues
      // Users can still use MetaMask and WalletConnect for all functionality
      /* 
      connectors.push(
        coinbaseWallet({
...
const createTransports = () => {
  const timeout = isBuildTime() ? 5000 : 10000 // Reduced timeout to prevent hanging
  const retryCount = isBuildTime() ? 1 : 2 // Reduced retries to prevent spam

  return {
    [brainarkChain.id]: http('https://rpc.brainark.online', {
      timeout,
      retryCount,
      retryDelay: 3_000, // Increased delay between retries
...
export const getTreasuryAddressForNetwork = (network: string, token: string): string => {
  const treasuryMap: Record<string, Record<string, string>> = {
    ...
    brainark: {
      BAK: process.env.NEXT_PUBLIC_BAK_BRAINARK_TREASURY || '0xC7A3e128f909153442D931BA430AC9aA55E9370D',
      // BrainArk network only has BAK token, USDT/USDC don't exist on this chain
      // Payments in USDT/USDC should be made on Ethereum/BSC/Polygon networks
    },
  }
