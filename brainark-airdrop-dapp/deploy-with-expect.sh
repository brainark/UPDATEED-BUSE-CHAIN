...
# BrainArk DApp Clean Production Deployment Script with expect
...
SSH_PASS="n8fXfH8C6Pu0c"

# SSH connection function with expect
ssh_cmd() {
    expect -c "
        set timeout 60
...
rsync_cmd() {
    expect -c "
        set timeout 300
        spawn rsync -avz -e \"ssh -o StrictHostKeyChecking=no\" $1 $SERVER_USER@$SERVER_IP:$2
        expect \"password:\"
        send \"$SSH_PASS\r\"
        expect eof
    "
...
if ! command -v expect &> /dev/null; then
    echo "❌ expect command not found. Let's try direct SSH approach..."
    exit 1
fi
...
cp -r src/hooks "$TEMP_DIR/src/" 2>/dev/null || true
cp -r src/utils "$TEMP_DIR/src/" 2>/dev/null || true
...
rsync_cmd "$TEMP_DIR/" "$SERVER_PATH/"
