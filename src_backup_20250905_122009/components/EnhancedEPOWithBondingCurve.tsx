...
import { useRealContractData } from '@/hooks/useRealContractData'
...
const EnhancedEPOWithBondingCurve: React.FC = () => {
  ...
  const switchToSelectedNetwork = async (networkKey: string) => {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => reject(new Error('Network switch timeout')), 15000) // 15 second timeout
    })
  ...
  const { stats: realStats, isLoading: realLoading, error: realError } = useRealContractData()
  
  // Use real contract data if available, fallback to EPO hook
  const currentStats = (realStats && realStats.contractBalance !== '0') ? realStats : epoStats
  const isStatsLoading = realLoading && epoLoading
        ...
        <div className="text-center mb-4">
          ...
          {(epoError || realError) && (
            <div className="text-sm text-red-600 mt-2">
              Contract Status: {realError ? 'Loading real data...' : (epoError || 'Connecting...')}
            </div>
          )}
        </div>
