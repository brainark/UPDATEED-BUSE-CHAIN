...
export default function AutoWalletConnection({ onConnectionChange }: AutoWalletConnectionProps) {
  ...
  // Get MetaMask provider (cached to prevent re-detection)
  ...
  // Check current network (throttled)
  const checkNetwork = useCallback(async () => {
    const now = Date.now()
    if (now - lastCheckRef.current < 1000) return // Throttle to max once per second
  ...
  // Check connection status (throttled)
  ...
  // Initialize wallet detection (only once)
  useEffect(() => {
  ...
  // Listen for account and network changes (only once)
  useEffect(() => {
    const provider = getProvider()
  ...
  const addNetwork = useCallback(async () => {
    ...
    try {
      ...
      if (error.code === 4001) {
        ...
        setError('Request already pending in MetaMask. Please check MetaMask and wait.')
