...
export function BrazilianShaderBackground({ 
  ...
  const variants = {
    classic: {
      colors: ['#FFD700', '#228B22', '#0047AB', '#FFD700'], // Brazilian flag colors
      gradientStops: [0, 0.33, 0.66, 1],
    },
    carnival: {
      colors: ['#FF1493', '#FF4500', '#9370DB', '#FFD700', '#FF69B4'],
      gradientStops: [0, 0.25, 0.5, 0.75, 1],
    },
    tropical: {
      colors: ['#32CD32', '#00FF7F', '#ADFF2F', '#7CFC00'],
      gradientStops: [0, 0.33, 0.66, 1],
    },
    sunset: {
      colors: ['#FF6347', '#FF4500', '#FFD700', '#FF69B4'],
      gradientStops: [0, 0.33, 0.66, 1],
    },
    ocean: {
      colors: ['#0080FF', '#00CED1', '#20B2AA', '#4682B4'],
      gradientStops: [0, 0.33, 0.66, 1],
    }
  }
      ...
      for (let x = 0; x < width; x += 2) {
        for (let y = 0; y < height; y += 2) {
          ...
          if (y + 1 < height && x < width) {
            ...
            data[nextRowIndex] = r
            data[nextRowIndex + 1] = g
            data[nextRowIndex + 2] = b
            data[nextRowIndex + 3] = a
            if (x + 1 < width) {
              data[nextRowIndex + 4] = r
              data[nextRowIndex + 5] = g
              data[nextRowIndex + 6] = b
              data[nextRowIndex + 7] = a
            }
          }
        }
      }
  ...
  return (
    <div className={`fixed inset-0 -z-10 ${className}`}>
      ...
      <motion.div
        ...
        style={{
          background: variant === 'classic' 
            ...
            : variant === 'carnival'
            ? 'radial-gradient(ellipse at center, transparent 0%, rgba(20,20,20,0.8) 70%)'
            : variant === 'tropical'
            ? 'radial-gradient(ellipse at center, transparent 0%, rgba(0,40,0,0.6) 70%)'
            : variant === 'sunset'
            ? 'radial-gradient(ellipse at center, transparent 0%, rgba(40,20,0,0.7) 70%)'
            : 'radial-gradient(ellipse at center, transparent 0%, rgba(0,20,40,0.7) 70%)',
      ...
      >
        ...
        <motion.div
          className="absolute top-1/4 left-1/4 w-32 h-32 rotate-45 bg-yellow-400/10"
          animate={{
            rotate: [45, 405],
            scale: [1, 1.2, 1],
          }}
        ...
        />
        <motion.div
          className="absolute bottom-1/3 right-1/3 w-24 h-24 rotate-45 bg-yellow-400/10"
          animate={{
            rotate: [45, -315],
            scale: [1, 0.8, 1],
          }}
        ...
        {/* Green circular elements */}
        <motion.div
          className="absolute top-1/2 right-1/4 w-40 h-40 rounded-full bg-green-500/10"
          animate={{
            scale: [1, 1.3, 1],
            x: [0, 50, 0],
          }}
        ...
        {/* Blue wave elements */}
        <motion.div
          className="absolute bottom-1/4 left-1/2 w-64 h-8 bg-blue-600/10 rounded-full"
          animate={{
            scaleX: [1, 1.5, 1],
            x: [-100, 100, -100],
          }}
