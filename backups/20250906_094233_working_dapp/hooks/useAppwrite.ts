...
export function useAppwrite(walletAddress?: string): UseAppwriteReturn {
  ...
  const loadUserData = async (address: string) => {
    ...
    try {
      // Load user via API
      const userResponse = await fetch(`/api/appwrite/user?walletAddress=${encodeURIComponent(address)}`)
      const { user: userData } = await userResponse.json()
      setUser(userData)

      if (userData) {
        // Load related data via API
        const [registrationResponse, tasksResponse] = await Promise.all([
          fetch(`/api/appwrite/airdrop?walletAddress=${encodeURIComponent(address)}`),
          fetch(`/api/appwrite/social-tasks?userId=${encodeURIComponent(userData.$id)}`)
        ])
        ...
        setReferrals([]) // Simplified for now
      }
    } catch (err) {
  ...
  const createUser = useCallback(async (address: string): Promise<User | null> => {
    ...
    try {
      // Create user via API
      const response = await fetch('/api/appwrite/user', {
        method: 'POST',
        headers: {
  ...
  const getAirdropStats = useCallback(async () => {
    try {
      // Try Appwrite first, fallback to local stats
      let response
      try {
        response = await fetch('/api/appwrite/airdrop?action=stats')
        if (!response.ok) throw new Error('Appwrite unavailable')
      } catch {
        response = await fetch('/api/fallback-stats')
      }
